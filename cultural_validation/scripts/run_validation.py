...
def city_centres_and_countries() -> pd.DataFrame:
    ...
    rows = []
    for tif in sorted((ROOT / "merge" / "200").glob("*.tif")):
...
def load_city(k: int, city: str):
    tif = ROOT / "merge" / str(k) / f"{city}.tif"
    ...
    for label, count in zip(vals, counts):
        p = ROOT / "clusters" / str(k) / "avg_feature" / f"{city}_patch_{label}.npy"
        if p.exists():
...
def analyse(merged: pd.DataFrame):
    ...
    fig.savefig(OUT / "ci_vs_cultural_distance.png", dpi=220)
    fig.savefig(OUT / "ci_vs_cultural_distance.pdf")
    plt.close(fig)
