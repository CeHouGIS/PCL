...
- `data/results/cultural_validation/city_pair_ci.csv`: directed and symmetric Covered Index (CI) for `K = 200, 500, 1000`.
- `data/results/cultural_validation/city_country_mapping.csv`: city coordinates used to calculate great-circle distance.
- `data/raw/historical_city_connection_layers_138x138.xlsx`: historical direct-tie and shared-regime matrices, and the country definition in `City List`.
...
```bash
python3 code/dyadic_dependence_validation/run_dyadic_dependence_validation.py
python3 code/dyadic_dependence_validation/plot_dyadic_dependence_validation.py
```

The scripts write aggregate data to `data/results/dyadic_dependence_validation/`, manuscript text to `docs/dyadic_dependence_validation/`, and figures to `images/dyadic_dependence_validation/`.
