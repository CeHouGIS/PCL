# Dyadic-dependence diagnostics and controlled MRQAP
...
4. **Controlled historical association.** Separate double-semi-partialling MRQAP models test direct ties and shared regimes after controlling for log geographic distance and same-country status. The reported coefficient is standardized; the permutation distribution is generated by simultaneous node-label permutations of the residualized historical predictor matrix.
...
The scripts generate aggregate CSV results and the K=1,000 four-panel figure in `output/`. The pair-level audit table and TIFF are generated locally but intentionally excluded from Git.
...
- After controlling for log distance and same-country status, shared regime remained positively associated with CI (`beta = 0.0691`, MRQAP `P = 0.0122`). Direct tie remained positive but was not statistically significant (`beta = 0.0339`, MRQAP `P = 0.1730`).
