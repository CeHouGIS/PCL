#!/usr/bin/env python3
"""Correlate the Shared Regime layer with symmetric Covered Index."""
...
HISTORICAL = ROOT / "data" / "raw" / "historical_city_connection_layers_138x138.xlsx"
CI_FILE = ROOT / "data" / "results" / "cultural_validation" / "city_pair_ci.csv"
OUT = ROOT / "data" / "results" / "shared_regime_validation"
SEED = 20260806
N_PERM = 9999
...
def main():
    ...
    matrix = pd.read_excel(HISTORICAL, sheet_name="Shared Regime Matrix", index_col=0)
    ...
    regime_matrix = matrix.loc[historical_names, historical_names].to_numpy(dtype=np.int8)
    upper = np.triu_indices(len(matched), 1)
    shared = regime_matrix[upper]
    ...
    for k, ci_matrix in ci_matrices.items():
        ...
        pearson = float(stats.pearsonr(shared, scores).statistic)
        spearman = float(stats.spearmanr(shared, scores).statistic)
        yes, no = scores[shared == 1], scores[shared == 0]
        ...
        cross_shared = shared[cross_country]
        cross_pearson = float(stats.pearsonr(cross_shared, cross_scores).statistic)
        cross_spearman = float(stats.spearmanr(cross_shared, cross_scores).statistic)
        for n, perm in enumerate(permutations):
            perm_shared = regime_matrix[np.ix_(perm, perm)][upper]
            pearson_null[n] = np.corrcoef(perm_shared, scores)[0, 1]
            spearman_null[n] = np.corrcoef(perm_shared, ranked_scores)[0, 1]
            cross_country_null[n] = np.corrcoef(perm_shared[cross_country], cross_scores)[0, 1]
        ...
        rows.append({
            ...
            "n_city_pairs": len(shared),
            "n_shared_regime_pairs": int(shared.sum()),
            "shared_regime_prevalence": float(shared.mean()),
            "mean_ci_shared_regime": float(yes.mean()),
            "mean_ci_no_shared_regime": float(no.mean()),
            "mean_difference": difference,
            "cohen_d": float(difference / pooled_sd),
            "roc_auc": auc_from_scores(shared, scores),
            "pearson_r": pearson,
            ...
            "cross_country_n_shared_regime_pairs": int(cross_shared.sum()),
            "cross_country_mean_ci_shared_regime": float(cross_scores[cross_shared == 1].mean()),
            "cross_country_mean_ci_no_shared_regime": float(cross_scores[cross_shared == 0].mean()),
            "cross_country_mean_difference": float(cross_scores[cross_shared == 1].mean() - cross_scores[cross_shared == 0].mean()),
    ...
    result.to_csv(OUT / "shared_regime_correlation_results.csv", index=False)
    np.savez_compressed(OUT / "shared_regime_permutation_nulls.npz", **nulls)
    metadata = {
        "layer": "Shared Regime Matrix",
        "definition": "Both cities share at least one historical or imperial regime; temporal overlap is not required.",
