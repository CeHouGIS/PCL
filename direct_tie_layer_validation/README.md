...
![Combined direct ties versus CI](output/direct_tie_vs_ci.png)

## Reproduce

Place `historical_city_connection_layers_138x138.xlsx` at the repository root, then run:

```bash
python direct_tie_layer_validation/scripts/calculate_direct_tie_correlation.py
python direct_tie_layer_validation/scripts/plot_direct_tie.py
```

The raw workbook, pair-level city mapping, and permutation null arrays are intentionally excluded from version control. Aggregate statistics and publication figures are included in `output/`.
